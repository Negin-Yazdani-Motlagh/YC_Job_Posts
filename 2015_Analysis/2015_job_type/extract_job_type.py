...
def extract_job_types(header: str) -> str:
    ...
    if re.search(r'\b(?:onsite|on\s*site|on-site|in\s*office|in-office)\b', header_lower):
...
def main():
    # Load posts
    with open("2015_Analysis/2015_job_posts.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    posts = data["YC"]["2015"]["comments"]
    ...
    output_file = Path("2015_Analysis/2015_job_type/2015_job_type_details.xlsx")
