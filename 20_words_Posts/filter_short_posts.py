...
def filter_short_posts():
    ...
    output_dir = Path("20_words_Posts")
    ...
    for month_year, month_data in data["YC"].items():
        ...
        # Filter comments with 20 words or less
        for comment in month_data["comments"]:
            if count_words(comment) <= 20:
    ...
    output_file = output_dir / "0_20_words_posts.json"
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(short_posts, f, indent=4)
    
    print(f"Found {sum(len(month['comments']) for month in short_posts['YC'].values())} posts with 20 words or less")
