...
def extract_job_types(header: str) -> str:
    ...
    # Only add On-site if Remote is not already detected
    if "Remote" not in found_types:
...
def main():
    # Load posts
    with open("2016_Analysis/2016_job_posts.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    posts = data["YC"]["2016"]["comments"]
    ...
    output_file = Path("2016_Analysis/2016_job_type/2016_job_type_details.xlsx")
