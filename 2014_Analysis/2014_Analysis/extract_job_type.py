...
def extract_job_types(header: str) -> str:
    ...
    if re.search(r'\b(?:onsite|on\s*site|on-site|in\s*office|in-office|local(?:\s+only)?)\b', header_lower):
...
def main():
    # Load posts
    with open("2014_Analysis/2014_job_posts.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    posts = data["YC"]["2014"]["comments"]
    ...
    output_file = Path("2014_Analysis/2014_job_type/2014_job_type_details.xlsx")
