...
def extract_job_types(header: str) -> str:
    ...
    if re.search(r'\b(?:onsite|on\s*site|on-site|in\s*office|in-office|local(?:\s+only)?|in\s*person|in-person|at\s*office|at\s*our\s*(?:office|location|headquarters))\b', header_lower):
...
def main():
    # Load posts
    with open("2013_Analysis/2013_job_posts.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    posts = data["YC"]["2013"]["comments"]
    ...
    output_file = Path("2013_Analysis/2013_job_type/2013_job_type_details.xlsx")
